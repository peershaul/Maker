...
void look_at_directory(FileOrDirectory *obj) {
  ...
  while ((entry = readdir(dir)) != NULL) {
    if (handmade_cmpstr("..", entry->d_name) ||
        handmade_cmpstr(".", entry->d_name))
      continue;
    item_amount++;
  }
  ...
  FileOrDirectory *children =
      (FileOrDirectory *)malloc(item_amount * sizeof(FileOrDirectory));
...
void read_ignore(const char *root_path, FileOrDirectory *ignore,
                 LinkedPaths *linked_path) {
  ...
  while (!end_of_file) {
    ...
    for (uint8_t i = 0; i < index - 1; i++)
      if (!isalnum(name_buff[i]) && !ispunct(name_buff[i]) &&
          !isspace(name_buff[i])) {
        is_readable = false;
        break;
      }
...
void search_for_c_files(CArray *arr, FileOrDirectory *root,
                        LinkedPaths *ignore_paths, MainSetting *settings) {
  if (!root->is_directory) {
    //printf("\"%s\": %d %d %d", root->path, handmade_str_ends_with(root->path, ".c"),
    //   handmade_str_contains(root->path, "."), handmade_str_contains(root->path, "cache"));
    if (handmade_str_ends_with(root->path, ".c") &&
        handmade_str_contains(root->path, "/.") == -1 &&
          handmade_str_contains(root->path, "cache") == -1) {
	  ...
	  if (handmade_str_contains(root->path, current_ignore_path->path) !=
              -1) {
	    not_ignored = false;
            break;
          }
...
int main(int argc, char *argv[]) {
  ...
  if (!read_arguments(argc, argv, &settings))
    return -1;
  printf("base path: \"%s\"\n", settings.base_path);
  ...
  FileOrDirectory *main_obj =
      (FileOrDirectory *)malloc(sizeof(FileOrDirectory));
  ...
  CArray* relevent = (CArray*) malloc(sizeof(CArray));
  relevent->elements = (FileOrDirectory *)malloc(C_ARR_LIMIT * sizeof(FileOrDirectory));
  relevent->length = 0;
  ...
  read_makefile(makefile_obj, &c_files, relevent, &makefile_variables,
                &makefile_uniques, &irelevent);

  for (uint16_t i = 0; i < relevent->length; i++)
    printf("%d: \"%s\"\n", i + 1, relevent->elements[i].path);

  write_to_makefile(main_obj, makefile_obj, relevent, makefile_variables,
                    makefile_uniques, irelevent);
